...
import com.uamother.bluetooth.ble.helper.BleManagerCallbacks;
...
public abstract class MultiBleManager<E extends BleManagerCallbacks> {
    ...
    private static final UUID CLIENT_CHARACTERISTIC_CONFIG_DESCRIPTOR_UUID = UUID.fromString("0000ffe0-0000-1000-8000-00805f9b34fb");
    ...
    protected final Map<String, BluetoothGatt> gattMap = new HashMap<String, BluetoothGatt>();
     ...
     */
    protected final boolean readCharacteristic(String address, final BluetoothGattCharacteristic characteristic) {
        final BluetoothGatt gatt = gattMap.get(address);
        if (gatt == null || characteristic == null)
            return false;
     ...
     * @return true if request has been sent
     */
    protected final boolean writeCharacteristic(String address, final BluetoothGattCharacteristic characteristic) {
        final BluetoothGatt gatt = gattMap.get(address);
        if (gatt == null || characteristic == null)
            return false;
    ...
    protected abstract class BleManagerGattCallback extends BluetoothGattCallback {
        private Map<String, Queue<Request>> mQueueMap = new HashMap<String, Queue<Request>>();
